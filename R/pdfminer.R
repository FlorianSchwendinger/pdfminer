...
#' @title Read a \code{PDF} document.
#' @description Extract  \code{PDF} document
#' @param file a character giving the name of the \code{PDF}-file the data are 
#'     to be read from.
...
#' @param method a character giving the data transfer method. Allowed values
...
#' @param strip_control a logical, if \code{TRUE} control characters are removed.
...
#' @param image_dir a character giving the path to the folder, where the images
#'     should be stored (default is \code{""}).
#' @param pyexe a character giving the path to the python executeable 
...
read.pdf <- function(file, pages = integer(), method = c('csv', 'sqlite', 'PythonInR'),
    laycntrl = layout_control(), encoding = 'utf8', strip_control = FALSE, 
    password = '', caching = TRUE, maxpages = Inf, rotation = 0L, image_dir = '',
    pyexe='python3') {
    ...
    assert(check_file_exists(file), check_integerish(pages), 
           check_character(encoding), check_logical(strip_control),
           check_character(password), check_logical(caching),
           check_integerish(maxpages), check_integerish(rotation),
           check_character(image_dir))
    ...
    if (method %in% c("csv", "sqlite")) {    
        ...
        if (!is_installed) {
            stop("there is no package called 'PythonInR'")
        }
